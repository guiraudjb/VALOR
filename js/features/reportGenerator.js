...
export function getStableHeaderHtml(
  config,
  brandHtml,
  customSubtitle = null,
...
export function getCurrentConfig() {
  ...
  const finalGranularity =
    reportMode === "mode-dep-com" || reportMode === "mode-epci"
      ? "com"
      : selectedGranularity;
...
export function getAggregatedDataMap(targetGranularity, page, config) {
  ...
  appState.userData.forEach((d) => {
    ...
    if (shouldAggregate && appState.sourceGranularity !== targetGranularity) {
      if (appState.sourceGranularity === "com" && targetGranularity === "dep") {
        ...
        appState.sourceGranularity === "dep" &&
        targetGranularity === "reg"
      ) {
        targetCode = appState.refData.depToReg.get(d.code);
      } else if (
        appState.sourceGranularity === "com" &&
        targetGranularity === "reg"
      ) {
  ...
  const m2 =
    config.selectedMetrics.length > 1 ? config.selectedMetrics[1] : null;
  ...
  if (config.shareBase === "view" && page && page.type !== "free") {
    ...
    codesInView = new Set(
      features.map((f) =>
        String(getCodeFromFeature(f, targetGranularity)).trim(),
      ),
    );
  }
  ...
  dataMap.forEach((obj, code) => {
    ...
    if (config.selectedMetrics.length > 0 || config.calcMode === "custom") {
      ...
      if (["simple", "top10", "flop10"].includes(mode)) val = obj[m1];
        ...
        val = config.selectedMetrics.reduce((s, m) => s + (obj[m] || 0), 0);
      else if (mode === "avg")
        val =
        ...
        val = refSum ? ((obj[m1] || 0) / refSum) * 100 : 0;
      else if (mode === "ratio")
        val = m1 && m2 && obj[m2] ? obj[m1] / obj[m2] : 0;
      else if (mode === "growth")
        val = m1 && m2 && obj[m1] ? ((obj[m2] - obj[m1]) / obj[m1]) * 100 : 0;
      ...
      // --- BLOC CUSTOM SÉCURISÉ ---
        ...
        try {
          ...
          const prec = parseInt(
            document.getElementById("input-precision")?.value || 2,
          );
    ...
    if (
      appState.dataFilter &&
      appState.dataFilter.active &&
      appState.dataFilter.operator !== "none"
    ) {
      ...
      if (valToTest === undefined || isNaN(valToTest)) {
        ...
        keep =
          valToTest >= Math.min(fv1, fv2) && valToTest <= Math.max(fv1, fv2);
      }
  ...
  if (["top10", "flop10"].includes(config.calcMode)) {
    let entries = Array.from(dataMap.entries()).filter(
      (e) => e[1]._computed !== undefined,
    );
...
export function buildDefaultStructure() {
  ...
  const defLabelSize =
    appState.defaultLabelSize ||
    (reportMode === "mode-dep-com" || reportMode === "mode-epci" ? 10 : 12);
  ...
  if (reportMode === "mode-nat-reg") {
    ...
    REGIONS_LIST.forEach((reg) =>
      newAutoPages.push(
        createMapPage(reg.label, { type: "region", code: reg.code }, "dep"),
      ),
    );
  } else if (reportMode === "mode-nat-del") {
    newAutoPages.push(createMapPage("France Entière", null, "dep"));
    appState.customDelegations.forEach((del) =>
      newAutoPages.push(
        createMapPage(
          del.label,
          { type: "delegation", list: del.depCodes },
          "dep",
        ),
      ),
    );
    ...
    newAutoPages.push(
      createMapPage(
        ...
        { type: "department", code: selectedDept },
        "com",
      ),
    );
    ...
    if (selectedEpci) {
      ...
      const communes = appState.refData.epciList
        .filter((r) => r.EPCI === selectedEpci)
        .map((r) => r.CODGEO);
      newAutoPages.push(
        createMapPage(
          `EPCI : ${epciName}`,
          { type: "epci", code: selectedEpci, list: communes },
          "com",
        ),
      );
    }
...
export function renderPageItem(page, config, dataMap, container) {
  ...
  if (page.type === "chart") {
    ...
    if (!isMultiScale) {
      ...
      filtered.forEach((f) => {
        ...
        if (granularity === "com")
          name =
            ...
            f.properties.nom_officiel ||
            f.properties.nom ||
            code;
        else if (granularity === "dep")
          name = DEP_NAMES[code] || f.properties.nom_officiel || code;
      ...
      safeNames = escapeHtml(
        JSON.stringify(
          page.chartConfig.metrics.map((m) =>
            m === "_computed" ? "Résultat" : m,
          ),
        ),
      );
      numColors =
        page.chartConfig.type === "pie"
          ? xLabels.length
          : page.chartConfig.metrics.length;
    } else {
      ...
      if (filtered.length > 0) {
        ...
        page.chartConfig.metrics.forEach((m, idx) => {
          ...
          if (
            count > 1 &&
            ["avg", "ratio", "share", "growth"].includes(config.calcMode)
          ) {
      ...
      if (page.chartConfig.scale === "reg_multi")
        entityName =
          REG_NAMES[page.chartConfig.entity] || page.chartConfig.entity;
      else if (page.chartConfig.scale === "dep_multi")
        entityName =
          DEP_NAMES[page.chartConfig.entity] || page.chartConfig.entity;
      else if (page.chartConfig.scale === "epci_multi") {
        const epciObj = appState.refData.epciList.find(
          (r) => r.EPCI === page.chartConfig.entity,
        );
        entityName = epciObj ? epciObj.LIBEPCI : page.chartConfig.entity;
      } else if (page.chartConfig.scale === "com_multi")
        entityName =
          appState.refData.communes.get(page.chartConfig.entity) ||
          page.chartConfig.entity;
      else if (page.chartConfig.scale === "del_multi") {
        const delegation =
          appState.customDelegations[parseInt(page.chartConfig.entity)];
      ...
      if (
        page.chartConfig.type === "pie" ||
        page.chartConfig.type === "line" ||
        page.chartConfig.type === "radar"
      ) {
        // Pour Camembert / Courbe / Radar : 1 seule série (l'entité), et N points/axes (les métriques)
        const xLabels = page.chartConfig.metrics.map((m) =>
          m === "_computed" ? "Résultat" : m,
        );
        safeX = escapeHtml(JSON.stringify(xLabels));
        ...
        const names = page.chartConfig.metrics.map((m) =>
          m === "_computed" ? "Résultat" : m,
        );
    ...
    const interpolator =
      PALETTE_SCALES[paletteKey] || PALETTE_SCALES["categorical"];
    const colorsArr = [];
  ...
  if (page.showTable) {
    ...
    if (features.length > 0) {
      generateTablePages(page, features, dataMap, config, granularity).forEach(
        (tp) => container.appendChild(tp),
      );
    }
  }
}

export function generateTablePages(
  page,
  features,
  dataMap,
  config,
  ...
  for (let i = 0; i < sorted.length; i += MAX_ROWS) {
    ...
    if (granularity === "com")
      headersHTML += `<th>Commune</th><th>Département</th>`;
    else if (granularity === "dep")
      headersHTML += `<th>Département</th><th>Région</th>`;
    ...
    if (page.type === "chart" && page.chartConfig && page.chartConfig.metrics) {
      ...
      page.chartConfig.metrics.forEach((m) => {
        const label =
          m === "_computed" ? `Résultat (${appState.calcMode})` : escapeHtml(m);
      ...
      if (
        page.richTable &&
        !["simple", "top10", "flop10"].includes(config.calcMode)
      ) {
      ...
      .map((f) => {
        ...
        if (granularity === "com") {
          ...
          const regCode =
            ...
            (appState.refData.depToReg
              ? appState.refData.depToReg.get(code)
              : null);
        ...
        if (
          ...
          page.chartConfig.metrics.forEach((m) => {
            ...
            const metricVal =
              val !== undefined ? formatValue(val, formatMode) : "-";
            const style =
              m === "_computed" ? "font-weight:bold; color:#000091;" : "";
          ...
          if (
            ...
            config.selectedMetrics.forEach((m) => {
              const metricVal =
                d[m] !== undefined ? formatValue(d[m], "simple") : "-";
...
export function generateReport() {
  ...
  const existingCharts = container.querySelectorAll(
  ...
  appState.pages.forEach((page) => {
    ...
    if (page.type === "free") {
      ...
      dataMap = getAggregatedDataMap(
        page.granularity || currentConfig.granularity,
        page,
        currentConfig,
      );
    }
...
window.updateEpciDepFilter = () => {
  ...
  Object.keys(DEP_NAMES)
    .sort()
    .forEach((dep) => {
      const depReg = appState.refData.depToReg
        ? appState.refData.depToReg.get(dep)
        : null;
...
window.confirmAddMapSlide = () => {
  ...
  const targetText = targetSel.options[targetSel.selectedIndex]
