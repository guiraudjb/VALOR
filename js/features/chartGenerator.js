...
export function openChartModal() {
  ...
  if (appState.availableMetrics.length === 0) {
    alert(
      "Veuillez d'abord charger un fichier CSV pour pouvoir créer un graphique.",
    );
    return;
  }
...
export function populateChartEpciReg() {
  ...
  document.getElementById("chart-select-epci-dep").innerHTML =
    '<option value="">-- Filtrer par département --</option>';
  document.getElementById("chart-select-entity").innerHTML =
...
export function checkPieWarning() {
  ...
  const checkedCount = document.querySelectorAll(
    'input[name="chart-metric-selection"]:checked',
  ).length;
...
export function saveChartSlide() {
  ...
  if (scale === "com_multi") {
    ...
    entity = document
      .getElementById("chart-input-com")
      .value.split(" ")[0]
      .trim();
  }
  ...
  const selectedMetrics = Array.from(
    document.querySelectorAll('input[name="chart-metric-selection"]:checked'),
  ).map((cb) => cb.value);
  ...
  if (!["france", "france_multi"].includes(scale) && !entity) {
  ...
  if (!title) {
    ...
    const metricLabel =
      selectedMetrics.length === 1
        ? selectedMetrics[0] === "_computed"
          ? "Résultat"
          : selectedMetrics[0]
        : scale.endsWith("_multi")
    ...
    if (scale === "france" || scale === "france_multi") {
      ...
      const epciName =
        document.querySelector("#chart-select-entity option:checked")
          ?.textContent || entity;
  ...
  } else if (scale === "dep_by_com") {
    focus = { type: "department", code: entity };
    gran = "com";
  } else if (scale === "epci_by_com") {
    const communes = appState.refData.epciList
      .filter((r) => r.EPCI === entity)
      .map((r) => r.CODGEO);
    focus = { type: "epci", code: entity, list: communes };
    gran = "com";
  }

  // -- NOUVEAUX MODES MULTIVALEURS --
  else if (scale === "france_multi") {
    gran = "dep";
    focus = null;
  } // On agrège tous les départements
  else if (scale === "reg_multi") {
    gran = "reg";
    focus = { type: "region", code: entity };
  } else if (scale === "dep_multi") {
    gran = "dep";
    focus = { type: "department", code: entity };
  ...
  const chartConfig = {
    ...
    palette,
    showMeanLine: showMean,
    showTrendline: showTrend,
    showMovingAvg: showMoving,
    highlightOutliers: showOutliers,
  };
...
window.editChartSlide = (id) => {
  ...
  document.getElementById("chart-opt-horizontal").checked =
    page.chartConfig.horizontal;
  document.getElementById("chart-select-palette").value =
    page.chartConfig.palette || "categorical";
  document.getElementById("chart-opt-mean").checked =
    page.chartConfig.showMeanLine || false;
  document.getElementById("chart-opt-trend").checked =
    page.chartConfig.showTrendline || false;
  document.getElementById("chart-opt-moving").checked =
    page.chartConfig.showMovingAvg || false;
  document.getElementById("chart-opt-outliers").checked =
    page.chartConfig.highlightOutliers || false;

  handleScaleChange();

  // Léger délai pour laisser le DOM construire les listes dynamiques (régions, EPCI...)
  setTimeout(() => {
    document.getElementById("chart-select-entity").value =
      page.chartConfig.entity;
    populateChartMetrics();

    document
      .querySelectorAll('input[name="chart-metric-selection"]')
      .forEach((cb) => {
