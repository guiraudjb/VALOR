...
window.insertDynamicTable = () => {
  ...
  for (let c = 0; c < cols; c++)
    tableHTML += `<th style="border:1px solid #ccc; padding:8px;">Titre ${c + 1}</th>`;
...
window.insertWysiwygImage = async (input) => {
  if (input.files && input.files[0]) {
    try {
      ...
      let width = prompt(
        "Quelle largeur pour cette image ? (ex: 50%, 400px, ou 100% par défaut)",
        "100%",
      );
...
export const saveCustomSlide = async () => {
  ...
  document
    .getElementById("modal-edit-slide")
  ...
  setTimeout(() => {
    ...
    const slideEl = document.querySelector(
      `#report-container .page[data-page-id="${finalId}"]`,
    );
...
window.insertDsfrIcon = (path, isPictogram) => {
  ...
  let width = prompt(
    "Quelle taille pour ce graphique ? (ex: 80px, 150px, 50%)",
    defaultWidth,
  );
...
window.insertTemplate = (templateKey) => {
  ...
  if (currentText.length > 20 && currentText !== "Votre texte ici...") {
    if (
      !confirm(
        "Attention, l'application d'un modèle va écraser votre contenu actuel. Voulez-vous continuer ?",
      )
    ) {
