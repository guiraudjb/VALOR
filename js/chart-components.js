...
class DSFRChart extends HTMLElement {
  ...
  connectedCallback() {
    this.innerHTML =
      '<div style="position: relative; width: 100%; height: 100%; min-height: 350px;"><canvas></canvas></div>';
    ...
    try {
      ...
      const datasets = ySeries.map((seriesData, index) => {
        ...
        if (chartType === "line" || chartType === "radar") {
          ...
          const pieColors =
            rawColors.length >= seriesData.length
              ? rawColors
              : Array(seriesData.length).fill(baseColor);
      ...
      console.error(
        "Erreur lors de la lecture des attributs du graphique :",
        error,
      );
      this.innerHTML =
        '<p style="color:#ce0500; font-weight:bold; padding: 1rem;">Erreur de chargement des données du graphique.</p>';
    }
  }
  ...
  renderChart(ctxParam, chartType, xLabels, rawDatasets, options = {}) {
    ...
    datasets.forEach((dataset) => {
      ...
      const baseColorSolid = Array.isArray(dataset.borderColor)
        ? dataset.borderColor[0]
        : dataset.borderColor || "rgb(0, 0, 145)";
    ...
    try {
      ...
      this.innerHTML =
        '<p style="color:#ce0500; font-weight:bold; padding: 1rem;">Erreur lors de la génération du graphique.</p>';
    }
  }
