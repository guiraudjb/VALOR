...
import {
  ...
  markAsDirty,
  clearDirty,
  updatePagesListUI as updateView,
} from "./viewUpdater.js";
...
export function setupUIListeners() {
  ...
  document.getElementById("btn-close-chart-modal").onclick = () => {
    ...
    m.classList.remove("fr-modal--opened");
    m.close();
  };
  ...
  document.getElementById("chart-select-epci-reg").onchange =
    populateChartEpciDep;
  document.getElementById("chart-select-epci-dep").onchange =
    populateChartEpciId;
  ...
  if (selectMode) {
    selectMode.onchange = (e) => {
      ...
      if (wDept)
        wDept.style.display = val === "mode-dep-com" ? "block" : "none";
  ...
  if (selectCalcMode) {
    selectCalcMode.addEventListener("change", (e) => {
      ...
      if (
        !["sum", "avg", "ratio", "growth"].includes(appState.calcMode) &&
        appState.selectedMetrics.length > 1
      ) {
        appState.selectedMetrics = [appState.selectedMetrics[0]];
      }
      if (
        ["ratio", "growth"].includes(appState.calcMode) &&
        appState.selectedMetrics.length < 2
      ) {
  ...
  const applyDataFilter = () => {
    ...
    appState.dataFilter.operator = selectFilterOp
      ? selectFilterOp.value
      : "none";
    appState.dataFilter.metric =
      selectFilterMetric && selectFilterMetric.value
        ? selectFilterMetric.value
        : "_computed";
    appState.dataFilter.value1 = inputFilterVal1
      ? parseFloat(inputFilterVal1.value)
      : 0;
    appState.dataFilter.value2 = inputFilterVal2
      ? parseFloat(inputFilterVal2.value)
      : 0;
    updateFilterControls();
    markAsDirty();
  };
  ...
  if (selectFilterOp)
    selectFilterOp.addEventListener("change", applyDataFilter);
  if (selectFilterMetric)
    selectFilterMetric.addEventListener("change", applyDataFilter);
  if (inputFilterVal1)
    inputFilterVal1.addEventListener("input", applyDataFilter);
  if (inputFilterVal2)
    inputFilterVal2.addEventListener("input", applyDataFilter);
  ...
  document.getElementById("btn-reset-structure").onclick =
    buildDefaultStructure;
    ...
    globalLogoInput.addEventListener("change", async (e) => {
      ...
      if (file) {
        try {
          ...
          console.error("Erreur de lecture du logo", err);
        }
      } else {
  ...
  if (btnAddSlide)
    btnAddSlide.onclick = () => {
      if (window.editPage) window.editPage();
    };
  ...
  if (btnAddAllCart)
    btnAddAllCart.onclick = () => {
      if (window.addAllToCart) window.addAllToCart();
    };
  ...
  if (btnOpenCart)
    btnOpenCart.onclick = () => {
      ...
      document
        ...
        .classList.add("fr-modal--opened");
      document.getElementById("modal-manage-cart").showModal();
    };

  // Transfert Panier -> Éditeur
  const btnCartToEditor = document.getElementById("btn-cart-to-editor");
  if (btnCartToEditor) {
    btnCartToEditor.onclick = () => {
      transferCartToEditor();
    };
  }
  ...
  document.getElementById("btn-close-modal").onclick = () => {
    modalSlide.classList.remove("fr-modal--opened");
    modalSlide.close();
  };
  ...
  document.getElementById("btn-open-del-modal").onclick = () => {
    renderDelegationList();
    resetDelegationForm();
    modalDel.classList.add("fr-modal--opened");
    modalDel.showModal();
  };
  document.getElementById("btn-close-del-modal").onclick = () => {
  ...
  document.getElementById("btn-cancel-delegation").onclick =
    resetDelegationForm;
  document.getElementById("btn-reset-delegations").onclick = () => {
    if (confirm("Réinitialiser délégations ?")) {
  ...
  if (btnFirst) btnFirst.onclick = () => navigateToSlide("first");
  const btnLast = document.getElementById("btn-nav-last");
  if (btnLast) btnLast.onclick = () => navigateToSlide("last");
  const btnGoto = document.getElementById("btn-nav-goto");
  if (btnGoto)
    btnGoto.onclick = () => {
      if (appState.pages.length === 0) return;
      const num = prompt(`Aller à la diapo (1 - ${appState.pages.length}) :`);
      if (num !== null) {
        const parsed = parseInt(num);
        if (!isNaN(parsed) && parsed >= 1 && parsed <= appState.pages.length)
          navigateToSlide(parsed - 1);
  ...
  if (reportContainer) {
    ...
    reportContainer.addEventListener("focusout", (e) => {
      if (e.target.classList.contains("editable-report-title")) {
        ...
        const pageId = pageEl
          ? parseInt(pageEl.getAttribute("data-page-id"))
          : null;
    ...
    reportContainer.addEventListener("keydown", (e) => {
      if (
        (e.target.classList.contains("editable-slide-title") ||
          e.target.classList.contains("editable-report-title")) &&
        e.key === "Enter"
      ) {
        e.preventDefault();
        e.target.blur();
    ...
    reportContainer.addEventListener("click", (e) => {
      ...
      if (e.target.classList.contains("edit-icon-trigger")) {
        ...
        const titleSpan = document.querySelector(
          `.editable-slide-title[data-page-id="${targetId}"]`,
        );
      ...
      if (e.target.classList.contains("edit-report-icon-trigger")) {
        ...
        const titleSpan = e.target
          .closest(".page-header")
          .querySelector(".editable-report-title");
...
if (btnWipe) {
  btnWipe.onclick = async () => {
    if (
      confirm(
        "⚠️ Supprimer définitivement toutes vos données locales (panier et réglages) ?",
      )
    ) {
...
window.navigateToSlide = function (target) {
  if (appState.pages.length === 0) return;
  let slideIndex = null;
  if (target === "first") slideIndex = 0;
  else if (target === "last") slideIndex = appState.pages.length - 1;
  else if (
    typeof target === "number" &&
    target >= 0 &&
    target < appState.pages.length
  )
    slideIndex = target;
  if (slideIndex !== null) {
    const targetSlideId = appState.pages[slideIndex].id;
    const targetElement = document.querySelector(
      `#report-container .page[data-page-id="${targetSlideId}"]`,
    );
    if (targetElement)
...
export function highlightAndScrollToNavItem(pageId) {
  ...
  items.forEach((item) => {
    if (item.getAttribute("data-page-id") === pageId.toString()) {
      if (item.classList.contains("active-page-item")) isAlreadyActive = true;
      else item.classList.add("active-page-item");
      targetItem = item;
...
function initFormulaAutocomplete() {
  ...
  input.addEventListener("input", () => {
    ...
    if (lastBracket !== -1 && val.indexOf("]", lastBracket) === -1) {
      ...
      const matches = appState.availableMetrics.filter((m) =>
      ...
      list.querySelectorAll(".suggestion").forEach((div) => {
        div.onclick = () => {
          input.value =
            val.substring(0, lastBracket) + "[" + div.textContent + "]";
...
function triggerCustomFormulaUpdate() {
  ...
  appState.selectedMetrics = detectedMetrics.filter((m) =>
    appState.availableMetrics.includes(m),
  );
