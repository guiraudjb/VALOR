...
import {
  ...
  updateMainBrand,
} from "../ui/viewUpdater.js"; // <--- AJOUT DE updateMainBrand
...
export function processEpciRef(csvText) {
  ...
  const startIndex = lines.findIndex(
    (l) => l.includes("CODGEO") || l.includes('"CODGEO"'),
  );
  const validCsv =
  ...
  window.Papa.parse(validCsv, {
    ...
    complete: (res) => {
      if (res.data)
        appState.refData.epciList = res.data.filter((r) => r.CODGEO && r.EPCI);
      populateEpciRegSelect();
    },
  });
}
  ...
  window.Papa.parse(fileOrText, {
    ...
    complete: (res) => {
      if (!res.data || res.meta.fields.length < 2) {
        alert("Format CSV invalide ou vide. 2 colonnes minimum requises.");
        return;
      }
      ...
      if (res.data.length > 0) {
        ...
        appState.userData = res.data.map((row) => {
          ...
          metricFields.forEach((metric) => {
            let rawVal =
              row[metric] !== undefined ? row[metric].toString() : "0";
            parsedRow[metric] =
              parseFloat(rawVal.replace(/\s/g, "").replace(",", ".")) || 0;
          });
          return parsedRow;
        });
...
function applyConfig(iniText) {
  ...
  if (conf.General) {
    if (conf.General.titre)
      document.getElementById("input-titre").value = conf.General.titre;
    if (conf.General.date)
      document.getElementById("input-date").value = conf.General.date;
    if (conf.General.source)
      document.getElementById("input-footer-left").value = conf.General.source;
    if (conf.General.credit)
      document.getElementById("input-footer-right").value = conf.General.credit;
  }
  if (conf.Marianne) {
    if (conf.Marianne.ligne1)
      document.getElementById("input-brand-1").value = conf.Marianne.ligne1;
    if (conf.Marianne.ligne2)
      document.getElementById("input-brand-2").value = conf.Marianne.ligne2;
  ...
  if (conf.Fichiers) {
    ...
    if (conf.Fichiers.communesRef)
      PATHS.communesRef = conf.Fichiers.communesRef;
...
export async function loadInitialData() {
  try {
    ...
    if (missing.length > 0) {
      alert(
        `Erreur dans config.ini : les chemins suivants sont manquants dans la section [Fichiers] :\n- ${missing.join("\n- ")}`,
      );
    ...
    alert(
      "Erreur réseau ou fichier corrompu lors du chargement des données. Consultez la console.",
    );
  }
}
